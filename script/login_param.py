...
class login(unittest.TestCase):
    ...
    def test01_get_img_code(self,type,status_code):
        ...
        if type=='float':
            ...
            r=str(random.randint(10000,900000))
        elif type=='char':
            r=''.join(random.sample("abcdefhijklmnopqw",8))
 ...
 # 获取短信验证码失败——未调用获取图片验证码错接口
    ...
    def test10_register(self,phone,pwd,imgVerifyCode,phoneCode,dyServer,invite_phone,status_code,status,description):
        ...
        logging.info("get sms code response = {}".format(response.json()))
        #
        response=self.login_api.register(self.session,phone,pwd,imgVerifyCode,phoneCode,
                                         dyServer,invite_phone,status_code,status,description)
    ...
    """    #输入必填项，注册成功
        ...
        def test15_register_sms_code_is_wrong(self):
            ...
            logging.info("register response = {}".format(response.json()))
            #对结果进行断言
            assert_utils(self,response,200,100,"短信验证码错误")
